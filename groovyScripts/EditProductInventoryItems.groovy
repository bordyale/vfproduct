...
if (product) {
    ...
    if (product.isVirtual && "Y".equals(product.isVirtual)) {
		...
		for (GenericValue order : purchaseOrders){
			Map<String,Object> e = new HashMap<String,Object>()
			e.orderDate=order.orderDate
			e.orderId=order.orderId
			shipmentReceipts = select("quantityAccepted").from("ShipmentReceipt").where("orderId", order.orderId).cache(false).queryList()
			totalReceived = 0.0
			if (shipmentReceipts != null){
				for (GenericValue shipmentReceipt : shipmentReceipts){
					totalReceived = shipmentReceipt.quantityAccepted + totalReceived
				}
				
			}
			
			e.quantity=order.quantity
			e.quantityReceived= totalReceived
			e.quantityToReceive=e.quantity - totalReceived
			hashMaps.add(e)
		}
		...
		context.purchaseOrders = hashMaps
    }
}
